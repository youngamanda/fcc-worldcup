...
echo "$($PSQL "SELECT DISTINCT(teams.name) FROM teams FULL JOIN games as wi ON teams.team_id=wi.winner_id FULL JOIN games as op ON teams.team_id=op.opponent_id WHERE (wi.year=2014 AND wi.round='Eighth-Final') OR (op.year=2014 AND op.round='Eighth-Final')")"
